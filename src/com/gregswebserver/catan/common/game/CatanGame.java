...
import com.gregswebserver.catan.common.game.gameplay.players.*;
...
import com.gregswebserver.catan.common.game.gamestate.GameStateEventType;
import com.gregswebserver.catan.common.game.gamestate.SharedState;
...
public class CatanGame implements ReversibleEventConsumer<GameEvent> {
    ...
    private LogicEvent createPlayerEvent(Username origin, PlayerEventType type, Object payload) {
        return new LogicEvent(this, LogicEventType.Player_Event, new PlayerEvent(origin, type, payload));
    }

    private LogicEvent createBoardEvent(TeamColor origin, BoardEventType type, Object payload) {
        return new LogicEvent(this, LogicEventType.Board_Event, new BoardEvent(origin, type, payload));
    }

    private LogicEvent createTeamEvent(TeamColor origin, TeamEventType type, Object payload) {
        return new LogicEvent(this, LogicEventType.Team_Event, new TeamEvent(origin, type, payload));
    }

    private LogicEvent createStateEvent(Object origin, GameStateEventType type, Object payload) {
        return new LogicEvent(this, LogicEventType.GameState_Event, new GameStateEvent(origin, type, payload));
    }
        ...
        switch (event.getType()) {
            ...
            case Turn_Advance:
                ...
                return new LogicEvent(this, LogicEventType.AND, Arrays.asList(
                    ...
                    new LogicEvent(this, LogicEventType.Player_Event, players.getPlayer(origin).getMatureEvent())
                ));
            ...
            case Build_Settlement:
                ...
                return new LogicEvent(this, LogicEventType.OR, Arrays.asList(
    ...
    private void test(LogicEvent event) throws EventConsumerException {
        ...
        switch (event.getType()) {
            ...
            case NOP:
                break;
            case Player_Event:
                players.test((PlayerEvent) payload);
                break;
            case Board_Event:
                board.test((BoardEvent) payload);
                break;
            case Team_Event:
                teams.test((TeamEvent) payload);
                break;
            case GameState_Event:
                state.test((GameStateEvent) payload);
                break;
        }
    }
    ...
    private LogicEvent execute(LogicEvent event) throws EventConsumerException {
        ...
        switch (event.getType()) {
            ...
            case NOP:
                break;
            case Player_Event:
                players.execute((PlayerEvent) payload);
                break;
            case Board_Event:
                board.execute((BoardEvent) payload);
                break;
            case Team_Event:
                teams.execute((TeamEvent) payload);
                break;
            case GameState_Event:
                state.execute((GameStateEvent) payload);
                break;
        }
        return event;
    }
    ...
    private void undo(LogicEvent event) throws EventConsumerException {
        try {
            switch (event.getType()) {
                case AND:
                case OR:
                    for (LogicEvent child : (List<LogicEvent>) event.getPayload())
                        undo(child);
                    break;
                case NOT:
                    undo((LogicEvent) event.getPayload());
                    break;
                case NOP:
                    break;
                case Player_Event:
