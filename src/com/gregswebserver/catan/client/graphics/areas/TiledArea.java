 ...
 * Resizable graphic
 */
public class TiledArea extends ScreenArea {

    private Graphic texture;

    public TiledArea(Point position, int priority, Graphic texture, Clickable clickable) {
    ...
    protected void render() {
        ...
        for (int x = 0; x < size.width; x += width) {
            for (int y = 0; y < size.height; y += height) {
                add(new StaticGraphic(new Point(x, y), 0, texture, clickable));
            }
        }
    }
}
